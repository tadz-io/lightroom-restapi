...
def profile():
    ...
    if "credentials" not in flask.session:
        return flask.render_template(
            "index.html", response="Please log in first"
        )
    else:
        ...
        # Store required parameters in a dictionary
        params = {"client_id": app.config["ADOBE_API_KEY"]}

        # Use requests library to send the GET request
        response = requests.get(
            profile_url,
            headers={
                "X-API-Key": app.config["ADOBE_API_KEY"],
                "Authorization": "Bearer {}".format(access_token)
            },
        )

        if response.status_code == 200:
            response_str = response.text[len('while (1) {}\n'):]
            return jsonify(json.loads(response_str))
            # return flask.render_template(
            #     "index.html", response=response.headers.get('Content-Type'))
        else:
            return flask.render_template(
                "index.html",
                response=f"profile could not be retrieved: {response.text}",
            )
